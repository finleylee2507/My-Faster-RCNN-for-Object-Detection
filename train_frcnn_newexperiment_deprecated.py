...
parser.add_option("--config_filename", dest="config_filename", help="Location to store all the metadata related to the training (to be used when testing).",
				  default="config_experiment.pickle")
...
classifier = nn.classifier(shared_layers, roi_input, 10, nb_classes=len(
	...
	while True:
		try:
			...
			for sample in sel_samples:
				...
				# test_loss = model_classifier.test_on_batch(
				# [X, np.expand_dims(X2[:, sample, :],axis=0)], [np.expand_dims(Y1[:, sample, :],axis=0), np.expand_dims(Y2[:, sample, :],axis=0) ])
				# print(test_loss)
			...
			losses[iter_num, 2] = loss_class[1] #detector_cls
			losses[iter_num, 3] = loss_class[2] #detector_regr
