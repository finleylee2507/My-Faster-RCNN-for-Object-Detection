...
pooling_output=model_classifier.get_layer("roi_pooling_conv_1").output 
m = Model(inputs=model_classifier.input, outputs=pooling_output) #extractor 
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
	...
	for jk in range(R.shape[0]//num_rois + 1):
		...
		roi_pooling_output=m.predict([F,ROIs])
		print(roi_pooling_output.shape)
