...
def calc_iou(R, img_data, C, class_mapping):
    ...
    for ix in range(R.shape[0]):
        ...
        for bbox_num in range(len(bboxes)):
            curr_iou = data_generators.iou(
                [gta[bbox_num, 0], gta[bbox_num, 2], gta[bbox_num, 1], gta[bbox_num, 3]], [x1, y1, x2, y2])
    ...
    if len(x_roi) == 0:
     ...
     # bboxes whose iou > C.classifier_min_overlap for all gt bboxes in 300 non_max_suppression bboxes
    X = np.array(x_roi)
    # one hot code for bboxes from above => x_roi (X)
    Y1 = np.array(y_class_num)
    # corresponding labels and corresponding gt bboxes
    Y2 = np.concatenate([np.array(y_class_regr_label),
...
def rpn_to_roi(rpn_layer, regr_layer, C, dim_ordering, use_regr=True, max_boxes=300, overlap_thresh=0.9):
    ...
    if dim_ordering == 'tf':
        ...
        A = np.zeros(
            (4, rpn_layer.shape[2], rpn_layer.shape[3], rpn_layer.shape[1]))
    ...
    all_boxes = np.reshape(A.transpose((0, 3, 1, 2)),
                           (4, -1)).transpose((1, 0))
